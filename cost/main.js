...
function mouseover(d) {
    if (d.name != "Empty") {
        ...
        for (var i = 0; i < ancestorArray.length; i++) {
            parents.push(ancestorArray[i].name)
        };
        ancestor = parents[0];

        var a = d3.selectAll(".legend")

        console.log(a[0])
        a.select("g#" + ancestor).style("opacity", 1)
...
function drawLegend() {
    ...
    length = d3.keys(colors).length - 1
    ...
    var g = legend.selectAll("g")
        .data(d3.entries(colors).slice(1))
        .enter().append("svg:g")
        .attr("id", function(d) {
            return d.key })
        .attr("class", "category")
        .attr("transform", function(d, i) {
            ...
            return "translate(" + (i * (li.w / length + li.s)) + ",25)";
        });
